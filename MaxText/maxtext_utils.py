...
def jit_and_compile(
  ...
  #with mesh, logical_axis_rules:
    ...
    #)
  #kwargs=dict(in_shardings=in_shardings)
  with mesh, logical_axis_rules:
  #with mesh:
    jitted = pjit(
        func,
...
def get_functional_train_with_signature_mfr(train_step, mesh, state_mesh_annotations, model, config, shaped_train_args):
  """Get the shardings (both state and data) for train_step"""
  print(f"MFR: get_functional_train_step")
  ...
  fn_in_partition_specs = (
        ...
        #state_mesh_annotations, #state(static)
        data_pspec, #batch
        None, #rng
    )
