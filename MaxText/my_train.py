...
def setup_mesh_and_model(config):
  ...
  print(f"MFR: checkpoint manager = {checkpoint_manager}")
...
def train_loop(config, state=None):
  ...
  print(f"MFR: call setup_train_loop")
  ...
  ) = maxtext_utils.get_functional_train_with_signature_mfr(train_step, mesh, state_mesh_annotations, model, config, shaped_train_args)
  ...
  print(f"MFR: trainstate_pspec = {in_shard_train[0]}")
  print(f"MFR: input_pspec = {in_shard_train[1]}")
  print(f"MFR: rnd_pspec = {in_shard_train[2]}")

 
  # postponed initial state fetch as we needed to evaluate sharding first
  print(f"MFR: initialize state with chosen sharding")
  #state = max_utils.initialize_state_with_sharding(init_state_partial, in_shard_train[0], init_rng)
  state = max_utils.initialize_state_with_sharding2(init_state_partial, mesh, in_shard_train[0], in_shard_train[2], init_rng)
  ...
  if config.compiled_trainstep_file != "":
    ...
    if True:
      ...
      p_eval_step = None
      def p_train_step2(state, inputs, prng_key):
        subset_extracted_state = (
            trees.extract_elements_matching_subset_structure(
                subset=shaped_args[0], superset=state
            )
        )
        subset_extracted_inputs = inputs
        #subset_extracted_inputs = (
        #    trees.extract_elements_matching_subset_structure(
        #        subset=shaped_args[1], superset=inputs
        #    )
        #)
        print(f"MFR: state = {state}")
        print(f"MFR: subset_extracted_state = {subset_extracted_state}")
        #print(f"MFR: subset_extracted_inputs = {subset_extracted_inputs}")
        return functional_train(subset_extracted_state, subset_extracted_inputs, prng_key)

      print("Train function already compiled!", flush=True)
    else:
  ...
  for step in np.arange(start_step, config.steps):
    print(f"Running training step {step} of {config.steps}")
    if step == first_profiling_step:
      print(f"Activate profiling")
      prof.activate()
    ...
    if config.eval_interval > 0 and step > start_step and step % config.eval_interval == 0:
      raise ValueError("MFR: evaluation?")
...
def main(argv: Sequence[str]) -> None:
  ...
  if False and nfig.monitor_goodput and jax.process_index() == 0:
    raise ValueError("MFR: main")
    logger_name = f'goodput_{config.run_name}'
